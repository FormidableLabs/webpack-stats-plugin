...
let IS_FIRST_START_DEVSERVER = true;
...
class StatsWriterPlugin {
  ...
  apply(compiler) {
    if (compiler.hooks) {
      compiler.hooks.emit.tapAsync("stats-writer-plugin", this.emitStats.bind(this));
    } else {
  ...
  emitStats(curCompiler, callback) {
    ...
    // dev server. Force write to disk
    if (curCompiler.compiler.outputFileSystem.constructor.name === "MemoryFileSystem") {

      if (IS_FIRST_START_DEVSERVER) {
        let file = path.join(curCompiler.compiler.options.output.path, this.opts.filename);

        IS_FIRST_START_DEVSERVER = false;
        fs.writeFile(file, statsStr, callback);
      }
